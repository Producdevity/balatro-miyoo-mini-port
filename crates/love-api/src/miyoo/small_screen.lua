-- Adapted from the PortMaster Balatro layout; see licenses/PortMaster.txt.
...
local BOTTOM_INSET = SAFE_MARGIN + 2.15
local HUD_ROW_H = 2.08
local OWNED_CARDS_Y = 2.5
...
local HUD_BLIND_W = HUD_INNER_W*0.28
local HUD_SCORE_W = HUD_INNER_W*0.165
local HUD_HAND_W = HUD_INNER_W*0.295
local HUD_META_W = HUD_INNER_W*0.26

local SCORE_SCALE = 0.92
local SCORE_TEXT_W = HUD_SCORE_W - 0.26
local TARGET_SCALE = 0.62
local HAND_TEXT_SCALE = 0.68
local HAND_LEVEL_MULT = 0.72
local HAND_NUM_SCALE = 0.96
local HAND_NUM_BOX_W = 2.0
local HAND_BOX_PAD = 0.06
local DOLLARS_SCALE = 0.74
local DOLLARS_W = 1.24
local BLIND_NAME_SCALE = 0.62
...
function CardArea:move(dt)
    if self == G.hand then
        ...
        -- Stock movement reserves 1.9 tiles while selecting. Replace that
        -- inset, retaining the same slide down when the hand is being scored.
        G.TILE_H = room_h - BOTTOM_INSET + 1.9
...
function CardArea:draw(...)
    ...
    if box and not box.small_screen_plate_hidden then
        ...
        if self == G.hand or self == G.jokers or self == G.consumeables then
            box.states.visible = false
        end
...
local function compact_main_menu_node(node)
    ...
    if config then
        if config.scale then config.scale = config.scale*0.9 end
        if config.minw and config.minw >= 2 then
            config.minw = config.minw*0.94
        end
        if config.maxw and config.maxw >= 2 then
            config.maxw = config.maxw*0.94
        end
        if config.padding and config.padding >= 0.1 then
            config.padding = config.padding*0.9
        end
    end
    if node.nodes then
...
function create_UIBox_main_menu_buttons(...)
    ...
    -- Keep the actions and language selector on separate rows in the 4:3 room.
    for _, node in ipairs(result.nodes) do
        node.n = G.UIT.R
        node.config.align = 'cm'
    end
    return result
end

local original_run_buttons = create_UIBox_buttons
function create_UIBox_buttons(...)
    local result = original_run_buttons(...)
    local sort_label = localize('b_sort_hand')
    local rank_label = localize('k_rank')
    local suit_label = localize('k_suit')
    local play_label = localize('b_play_hand')
    local discard_label = localize('b_discard')

    local function enlarge_run_button_text(node)
        if type(node) ~= 'table' then return end
        if node.n == G.UIT.T and node.config then
            local text = node.config.text
            if text == sort_label then
                node.config.scale = math.max(node.config.scale or 0, 0.48)
            elseif text == rank_label or text == suit_label then
                node.config.scale = math.max(node.config.scale or 0, 0.42)
            elseif text == play_label or text == discard_label then
                node.config.scale = math.max(node.config.scale or 0, 0.52)
            end
        end
        if node.nodes then
            for _, child in ipairs(node.nodes) do enlarge_run_button_text(child) end
        end
    end

    enlarge_run_button_text(result)
    return result
end
...
function set_screen_positions()
    if G.STAGE == G.STAGES.RUN then
        ...
        -- A scoring card rises by HIGHLIGHT_H. Keep its highest position below
        -- the owned cards, including the half-height difference of the area.
        G.play.T.y = OWNED_CARDS_Y + G.CARD_H + SAFE_MARGIN +
            G.HIGHLIGHT_H + (G.CARD_H - G.play.T.h)/2
...
function FixedText:draw()
    ...
    local layout_unit = (font.LAYOUT_FONTSCALE or font.FONTSCALE)/G.TILESIZE
    ...
    for _, part in ipairs(self.parts) do
        if part.width and part.width > 0 then
            ...
            love.graphics.draw(part.drawable,
                x + font.TEXT_OFFSET.x*part_scale*layout_unit,
                0.5*(self.T.h - height) + font.TEXT_OFFSET.y*part_scale*layout_unit,
    ...
    local controls = {n=G.UIT.C, config={align='cm', padding=0.08}, nodes={
        {n=G.UIT.R, config={id='next_round_button', align='cm', minw=3.0,
            ...
            {n=G.UIT.R, config={align='cm', padding=0.04,
                ...
                {n=G.UIT.R, config={align='cm', maxw=2.5}, nodes={
                    label(localize('b_next_round_1'), 0.5)
                }},
                {n=G.UIT.R, config={align='cm', maxw=2.5}, nodes={
                    label(localize('b_next_round_2'), 0.5)
                }}
            }}
        }},
        {n=G.UIT.R, config={align='cm', minw=3.0, minh=1.5,
            ...
            {n=G.UIT.R, config={align='cm', padding=0.04,
                ...
                {n=G.UIT.R, config={align='cm', maxw=2.5}, nodes={
                    label(localize('k_reroll'), 0.5)
                }},
                {n=G.UIT.R, config={align='cm'}, nodes={
                    label(localize('$'), 0.58),
                    {n=G.UIT.T, config={ref_table=G.GAME.current_round,
                        ref_value='reroll_cost', scale=0.68, colour=G.C.WHITE,
                        shadow=true}}
                }}
            }}
        }}
    ...
    local meta = {n=G.UIT.C, config={align='cm', minw=HUD_META_W, minh=row_h,
        ...
        {n=G.UIT.R, config={align='cm', minh=0.72}, nodes={
            ...
            {n=G.UIT.C, config={id='hud_hands', align='cm', minw=1.0,
                ...
                label('H', 0.5, G.C.BLUE),
                gap(0.04),
                dyn(G.GAME.current_round, 'hands_left', G.C.BLUE, 0.76, 'hand_UI_count')
            }},
            gap(0.06),
            {n=G.UIT.C, config={align='cm', minw=1.0, minh=0.66,
                ...
                label('D', 0.5, G.C.RED),
                gap(0.04),
                dyn(G.GAME.current_round, 'discards_left', G.C.RED, 0.76, 'discard_UI_count')
            }}
        }},
        {n=G.UIT.R, config={align='cm', minh=0.72}, nodes={
            {n=G.UIT.C, config={id='hud_ante', align='cm', minw=1.72,
                minh=0.66, padding=0.03, r=0.08, colour=inset}, nodes={
                label('A', 0.5, G.C.IMPORTANT),
                gap(0.04),
                dyn(G.GAME.round_resets, 'ante', G.C.IMPORTANT, 0.76, 'ante_UI_count'),
                label('/', 0.42),
                {n=G.UIT.T, config={ref_table=G.GAME, ref_value='win_ante',
                    scale=0.5, colour=G.C.WHITE, shadow=true}}
            }},
            gap(0.06),
            {n=G.UIT.C, config={align='cm', minw=1.14, minh=0.66,
                padding=0.03, r=0.08, colour=inset}, nodes={
                label('R', 0.5, G.C.IMPORTANT),
                gap(0.04),
                dyn(G.GAME, 'round', G.C.IMPORTANT, 0.76, 'round_UI_count')
            }},
            gap(0.06),
            ...
            gap(0.06),
            {n=G.UIT.C, config={id='options_button', align='cm', minw=0.48,
                minh=0.66, padding=0.03, r=0.08, colour=G.C.ORANGE,
                hover=true, button='options', shadow=true,
                focus_args={button='start', orientation='bm'}}, nodes={
                label('II', 0.42)
            }}
        }}
    }}
...
function create_UIBox_HUD_blind()
    ...
    return {n=G.UIT.ROOT, config={id='HUD_blind', func='HUD_blind_visible',
        ...
        {n=G.UIT.R, config={align='cm', minh=0.27, maxw=inner_w,
            ...
            {n=G.UIT.T, config={ref_table={val=''}, ref_value='val', scale=0.4,
            ...
            {n=G.UIT.T, config={id='HUD_blind_debuff_1',
                ...
                scale=0.4, colour=G.C.UI.TEXT_LIGHT, func='HUD_blind_debuff'}}
        }},
        {n=G.UIT.R, config={align='cm', minh=0.27, maxw=inner_w,
            colour=G.C.DYN_UI.DARK}, nodes={
...
function Game:update_selecting_hand(dt)
    ...
    if buttons and not buttons.small_screen_bottom_anchor then
        buttons.small_screen_bottom_anchor = true
        buttons:set_alignment({major=G.ROOM_ATTACH, type='bmi',
            offset={x=0, y=-SAFE_MARGIN}, bond='Strong'})
        buttons:align_to_major()
        buttons:hard_set_VT()
    end
