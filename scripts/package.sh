...
GAME=${1:-${BALATRO_GAME:-$ROOT/.tmp/portmaster_port/balatro/Balatro}}
...
if [ "${PREPARE_AUDIO:-1}" = 1 ]; then
    ...
    cargo run --locked --quiet --release --manifest-path "$ROOT/Cargo.toml" \
        -p love-api --example prepare-audio -- "$GAME" "$PCM_CACHE_DIR"
fi
...
if [ -f "$ROOT/.tmp/portmaster_port/balatro/cover.png" ]; then
    mkdir -p "$STAGING/Roms/PORTS/Imgs"
    cp "$ROOT/.tmp/portmaster_port/balatro/cover.png" "$STAGING/Roms/PORTS/Imgs/Balatro.png"
fi
